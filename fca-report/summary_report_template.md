# FCA Transaction Report — Reporting Period: 01 Mar 2026 – 31 May 2026

> **DRAFT TEMPLATE** — figures to be populated from the firm's system-of-record once the
> production data source for the period is confirmed. All values in **GBP equivalent** unless
> stated. Foreign-currency legs converted at the transaction's applied FX rate.
> _This is a working template; your Compliance Officer must confirm it maps to the correct
> FCA RegData return for the firm's permissions before submission._

## A. Firm & report details
| Field | Value |
|---|---|
| Firm name | Remitm Limited |
| FCA Firm Reference Number (FRN) | _____________ |
| Permissions / category | Payment Institution (money remittance) — confirm |
| Reporting period | 01 Mar 2026 – 31 May 2026 |
| Currency basis | GBP equivalent |
| Prepared by | _____________ |
| Date prepared | _____________ |
| Reviewed/approved by (MLRO/Compliance) | _____________ |

## B. Headline figures
| Metric | Value |
|---|---|
| Total transactions (count) |  |
| Total value (GBP) |  |
| Average transaction value (GBP) |  |
| Median transaction value (GBP) |  |
| Largest single transaction (GBP) |  |
| Active sending customers |  |
| New customers onboarded in period |  |
| Total fees / firm income (GBP) |  |

## C. Volume & value by month
| Month | Transactions | Value (GBP) | Avg (GBP) | Active customers |
|---|---|---|---|---|
| Mar 2026 |  |  |  |  |
| Apr 2026 |  |  |  |  |
| May 2026 |  |  |  |  |
| **Total** |  |  |  |  |

## D. By destination country / corridor
| Destination country | Corridor (e.g. GBP→GHS) | Transactions | Value (GBP) | % of value | High-risk jurisdiction (Y/N) |
|---|---|---|---|---|---|
|  |  |  |  |  |  |

## E. By currency (receive)
| Receive currency | Transactions | Value (GBP) |
|---|---|---|
|  |  |  |

## F. By payout method
| Payout method | Transactions | Value (GBP) | % of value |
|---|---|---|---|
| Bank deposit |  |  |  |
| Mobile wallet |  |  |  |
| Cash pickup |  |  |  |

## G. By status
| Status | Transactions | Value (GBP) |
|---|---|---|
| Completed / Paid |  |  |
| Pending |  |  |
| Cancelled |  |  |
| Failed |  |  |
| Refunded |  |  |

## H. Transaction-size distribution (AML)
| Size band (GBP) | Transactions | Value (GBP) |
|---|---|---|
| < 100 |  |  |
| 100 – 999.99 |  |  |
| 1,000 – 9,999.99 |  |  |
| ≥ 10,000 |  |  |

## I. Large / threshold transactions (Enhanced Due Diligence)
_List every transaction ≥ £10,000 or ≥ €15,000, plus any aggregated/linked transfers that
breach the threshold in aggregate._
| Reference | Date | Customer | Beneficiary country | Value (GBP) | EDD performed (Y/N) | Notes |
|---|---|---|---|---|---|---|
|  |  |  |  |  |  |  |

## J. High-risk jurisdiction exposure
| Jurisdiction (FATF high-risk / sanctioned) | Transactions | Value (GBP) | % of total value |
|---|---|---|---|
|  |  |  |  |
| **Total high-risk** |  |  |  |

## K. Customer metrics
| Metric | Value |
|---|---|
| Total active senders |  |
| New customers onboarded |  |
| Avg transactions per customer |  |
| KYC tier mix (TIER_1 / TIER_2 / TIER_3) |  |
| Customers in high-risk countries |  |

## L. Firm income
| Metric | Value (GBP) |
|---|---|
| Total fees charged |  |
| FX margin income (if applicable) |  |
| Total income |  |

## M. Financial-crime summary
| Metric | Value |
|---|---|
| SARs filed in period |  |
| Transactions flagged / under review |  |
| EDD cases completed |  |
| Refunds / reversals (count, value) |  |
| Declined / blocked (sanctions, fraud) |  |

## N. Declaration
> I confirm the figures in this report are, to the best of my knowledge, complete and accurate
> and reconcile to the firm's transaction records for the period.

| Name | Role | Signature | Date |
|---|---|---|---|
|  | MLRO / Compliance Officer |  |  |
